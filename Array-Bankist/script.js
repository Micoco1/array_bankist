...
const displayMovement = function(movements, sort = false){
  ...
  movs.forEach(function(mov, i){
    ...
    <div class="movements__row">
          ...
          movements__type--${type}">2${i + 1} ${type}</div>        
...
/* 
// deposits only Positive numbers
...
console.log('mico'.at(0));
*/

// Looping array using for each method forEach()
/*
const movements = [200, 450, -400, 3000, -650, -130, 70, 1300];
...
// const movementsUSD = movements.map(function(map){
//   return mov * eurToUsd;
// })
...
labelBalance.addEventListener('click', function(){
  const movementsUI = Array.from(document.
   ...
   // movementsUI2 = [...querySelectorAll('.movements__value')];
});
*/
...
const juliaCopy = checkDogs.dogsJulia.splice(1, 2)
//console.log(juliaCopy);

const allData = {
  correctedData: juliaCopy.concat(checkDogs.dogsKate), 
  
}

console.log(allData.correctedData);

for (const [key, data] of allData.correctedData.entries()) {
  if (data >= 3) {
    console.log(`Dog number ${key + 1} is an adult, and ${data} years old`);
  } else {
...
if the dog is <= 2 years old, humanAge = 2 * dogAge. If the dog 
...
// 1.
const calcAverageHumanAge = dogAge.map(age => {
  if(age <= 2)
  return 2 * age
  else 
  return 16 + age * 4
...
// 2.
const moreHumanYears = calcAverageHumanAge.filter(age => age > 18);
console.log(moreHumanYears);

// 3.
const calcAverage = moreHumanYears.reduce((acc, age) => 
acc + age / moreHumanYears.length, 0)
...
const average = calcAverageHumanAge
  .reduce((acc, age) => acc + age, 0) / calcAverageHumanAge.length

  console.log(average);

console.log('---------- ANSWER -----------');

const calcAverageHumanAge1 = ages => ages
  .map(age => (age <= 2 ? 2* age : 16 + age * 4))
  .filter(age => age >= 18)
...
like this: "Matilda and Alice and Bob's dogs eat too much!" and "Sarah 
...
dogs.forEach(dog => (dog.recommendedFood = 
...
console.log(`Sarah's dog is eating too ${dogSarah.curFood > 
  dogSarah.recommendedFood? 'much' : 'little'}`);
...
// 6.
const okayFood = dog => dog.curFood > (dog.recommendedFood * 0.90) && 
